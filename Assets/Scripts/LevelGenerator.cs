...
public class LevelGenerator : MonoBehaviour
{
	...
	bool[,] GenerateMazeData()
	{
		...
		int numberOfHoles = 0;
		while (numberOfHoles < 4)
		{
			int x = Mathf.Clamp(Random.Range(1, 38), 1, mazeSize - 2), z = Mathf.Clamp(Random.Range(1, 38), 1, mazeSize - 2);
			if (data[z, x] == false) // && CheckIfInArray(z, x)
			{
				holes[numberOfHoles] = new int[] { z, x };
				numberOfHoles++;
			}
		}
		//Debug.Log(holes[0][1]);
	...
	bool CheckIfInArray(int z, int x)
	{
		for (int i = 0; i < holes.Length; i++)
		{
			//Debug.Log(holes[3][0] + " " + holes[3][1]);
			//Debug.Log(mapData[z][x]);
			if (holes[i][0] == z && holes[i][1] == x && mapData[z, x] == false)
			{
				return false;
			}
		}
