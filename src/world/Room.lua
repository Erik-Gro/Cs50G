...
function Room:update(dt)
    ...
    for c = #self.entities, 1, -1 do
        local z = self.entities[c]
        if self.entities[c].dead then
            table.remove(self.entities, c)
            f = true
        end
    end
    if f == true then
        for c = 1, #self.entities, 1 do
            self.entities[c].stateMachine = StateMachine {
                ['walk'] = function() return EntityWalkState(self.entities[c], self.dungeon) end,
                ['idle'] = function() return EntityIdleState(self.entities[c], self.dungeon) end
            }
            self.entities[c]:changeState('walk')
        end
    end
    for c = #self.entities, 1, -1 do
        local z = self.entities[c]
    ...
    -- for k, pair in pairs(self.entities) do
    --     if pair.dead then
    ...
    for k, object in pairs(self.objects) do
        object:update(dt)
        if object.type == 'tiles' and object.delete and object.brokentime <= 0 then
