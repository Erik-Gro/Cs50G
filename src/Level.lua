...
function Level:init()
    ...
    function beginContact(a, b, coll)
        ...
        -- if types[1] == 'Player' and types[2] == 'Player' then
        --     -- grab the bodies that belong to the player and alien
        --     local playerFixture = a
        --     local alienFixture = b

        --     -- destroy the alien if player is traveling fast enough
        --     local velX1, velY1 = playerFixture:getBody():getLinearVelocity()
        --     local sumVel1 = math.abs(velX1) + math.abs(velY1)

        --     local velX2, velY2 = playerFixture:getBody():getLinearVelocity()
        --     local sumVel2 = math.abs(velX2) + math.abs(velY2)

        --     if sumVel1 > 20 or sumVel2 > 20 then
        --         table.insert(self.destroyedBodies, alienFixture:getBody())
        --         table.insert(self.destroyedBodies, alienFixture:getBody())
        --     end
        -- end
...
function Level:update(dt)
    ...
    --     if self.alien[i].body:isDestroyed() then
    ...
    if self.launchMarker.launched then
        local allbirds = #self.launchMarker.alien
        local stoppedbirds = 0
        for index, value in ipairs(self.launchMarker.alien) do
            local xPos, yPos = value.body:getPosition()
            local xVel, yVel = value.body:getLinearVelocity()

            -- if we fired our alien to the left or it's almost done rolling, respawn
            if xPos < 0 or (math.abs(xVel) + math.abs(yVel) < 1.5) then
                stoppedbirds = stoppedbirds + 1
                value.body:destroy()
                if allbirds == stoppedbirds then
                    self.launchMarker = AlienLaunchMarker(self.world)
                end
    ...
    for i = #self.launchMarker.alien, 1, -1 do
        if self.launchMarker.alien[i].body:isDestroyed() then
            table.remove(self.launchMarker.alien, i)
            gSounds['kill']:stop()
            gSounds['kill']:play()
        end
