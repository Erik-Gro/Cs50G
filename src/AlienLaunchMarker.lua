...
function AlienLaunchMarker:update(dt)
    ...
    if not self.launched then
        ...
        if love.keyboard.isDown('space') and self.specialused == false then
            self.specialused = true
            --self.alien[1]:special()
            local alien = Alien(self.world, 'round', self.alien[1].body:getX(), self.alien[1].body:getY() - 50, 'Player')
            x, y = self.alien[1].body:getLinearVelocity()
        ...
        for i = #self.alien, 1, -1 do
            if self.alien[i].body:isDestroyed() then
                table.remove(self.alien, i)
                gSounds['kill']:stop()
                gSounds['kill']:play()
            end
        end
    end
    -- if #self.alien > 0 then
    --     for index, value in ipairs(self.alien) do
    --         if value:isDestroyed() then
    --             table.remove(self.alien, index)
    --         end
    --     end
    -- end
end
