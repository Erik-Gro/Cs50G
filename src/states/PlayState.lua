...
function PlayState:update(dt)
    if Clicked == true then
        self.highlightedTile = self.board.tiles[4][4]
        Clicked = false
    end
                ...
                if self.board:calculateMatches() then
                    ...
                    for a = 1, 8, 1 do
                        ...
                        for z = 1, 8, 1 do
                            ...
                            if a - 1 >= 1 then
                                self.highlightedTile = self.board.tiles[a][z]
                                newTile = self.board.tiles[a - 1][z]
                                tempX = self.highlightedTile.gridX
                                tempY = self.highlightedTile.gridY
                                self.highlightedTile.gridX = newTile.gridX
                                self.highlightedTile.gridY = newTile.gridY
                                ...
                                if z - 1 >= 1 then
                                    self.highlightedTile = self.board.tiles[a][z]
                                    newTile = self.board.tiles[a][z - 1]
                                    tempX = self.highlightedTile.gridX
                                    tempY = self.highlightedTile.gridY
                                    self.highlightedTile.gridX = newTile.gridX
                                    self.highlightedTile.gridY = newTile.gridY
                                    ...
                                    if Breakout == false and a == 8 and z == 8 then
                                        self.board = Board(VIRTUAL_WIDTH - 272, 16, self.level)
                                    end
                                    self.highlightedTile = nil
                                end
    ...
    if matches then
        ...
        for k, match in pairs(matches) do
            ...
            for j, tile in pairs(match) do
                if tile.shiny == 1 then
                    for j, tile in pairs(match) do
                        self.score = self.score - tile.variety * 3
                    end
                    if tile.explode == 'w' then
                        for i = 1, 8, 1 do
                            table.insert(newmatch, self.board.tiles[tile.gridY][i])
                            self.score = self.score + tile.variety * 3
                        end
                    elseif tile.explode == 'h' then
                        for i = 1, 8, 1 do
                            table.insert(newmatch, self.board.tiles[i][tile.gridX])
                            self.score = self.score + tile.variety * 3
                        end
                    end
                else
