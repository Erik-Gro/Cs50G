...
function PlayState:init()
    ...
    --     level = self.level
    -- })
    -- for key, value in pairs(self.tileMap.tiles) do
    --     for i = 1, #value, 1 do
    --         if value[i].id == TILE_ID_GROUND then
    --             self.player.x = (i - 1) * 16
    --             break
    --         end
    --     end
    -- end
...
function PlayState:update(dt)
    Timer.update(dt)
    if self.player.x == (self.width - 1) * 16 and self.player.blockWasDestroyed then
        gStateMachine:change('play', self.lvl + 1)
    end
